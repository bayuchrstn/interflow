    ...
    $("#myDropzone").dropzone({
        ...
        error: function (file, response) {
            ...
            if (ord == "Remove file") {
                ...
                alert('File foto melebihi Max File Upload 4.5 MB'); // 4MB
                $('#myDropzone').removeClass('dz-started');
            } else {
                $(file.previewElement).remove();
            }
...
function delete_data(id) {
    bootbox.confirm("Yakin akan menyembunyikan properti ini ?", function (event) { // menghapus data 
        if (event == true) {
            $.ajax({
