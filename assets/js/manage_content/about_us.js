...
function load_table() {
	...
	$("#table1").DataTable({
        ...
        ajax : {
            ...
            url: base_url + 'admin/Manage_content/dt_about_us',
...
function save_data() {
	...
	$.ajax({
		...
		url : base_url+"admin/Content_trx/ajax_simpan_about_us",
...
function get_id(id) {
	...
	$.ajax({
		...
		url: base_url + "admin/Content_trx/get_about_us_by_id/" + id,
		...
		success: function (data) {
			...
			$('#old_img').show();
			$('#id').val(data.id); 
			$('#profil').val(data.profil_perusahaan);
			$('#no_siup').val(data.nmr_siup);
			$('#img_name').val(data.img_profil); 
			$('#old_img').html('<div class="form-group row"> <label class="col-form-label col-md-2"></label></label> <div class="col-md-8">  <div class="input-group"> <a href="' + data.img_url_profil + '"><img src="' + data.img_url_profil + '" style="width:100px;"> </a> </div> </div> </div>'); // Foto Lama
	...
	bootbox.confirm("Yakin akan menghapus data ?", function(event){
		...
		if (event == true) {
			...
			$.ajax({
				...
				url : base_url+'admin/Content_trx/hapus_about_us/'+id,
