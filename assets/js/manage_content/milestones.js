...
function load_table() {
	...
	$("#table1").DataTable({
        ...
        ajax : {
            ...
            url: base_url + 'admin/Manage_content/dt_milestones',
...
function save_data() {
	...
	$.ajax({
		...
		url : base_url+"admin/Content_trx/ajax_simpan_milestones",
...
function get_id(id) {
	...
	$.ajax({
		...
		url: base_url + "admin/Content_trx/get_milestones_by_id/" + id,
	...
	bootbox.confirm("Yakin akan menghapus data ?", function(event){
		...
		if (event == true) {
			...
			$.ajax({
				...
				url : base_url+'admin/Content_trx/hapus_milestones/'+id,
