...
function load_table() {
	...
	$("#table1").DataTable({
        ...
        responsive: true,
        ajax : {
            url: base_url + 'admin/Manage_content/dt_footer',
            type: "GET"
        },
        columnDefs: [
            {
...
function save_data() {
	...
	$.ajax({
		url : base_url+"admin/Content_trx/ajax_simpan_footer",
		type : "POST",
		data : formData,
		dataType : "JSON",
		contentType: false,
...
function get_id(id) {
	$.ajax({
		url: base_url + "admin/Content_trx/get_footer_by_id/" + id,
		dataType: "json",
		beforeSend: function () {
			blockUI();
		},
		...
		success: function (data) {
			...
			$('#instagram').val(data.instagram_url);
			$('#email_name').val(data.email_name);
			$('#facebook_name').val(data.facebook_name);
			$('#instagram_name').val(data.instagram_name);
		},
		error: function () {
...
function delete_data(id) {
	bootbox.confirm("Yakin akan menghapus data ?", function(event){
		if (event == true) {
			$.ajax({
				url : base_url+'admin/Content_trx/hapus_footer/'+id,
				dataType : "json",
				beforeSend : function(){
					blockUI();
				},
