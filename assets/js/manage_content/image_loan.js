...
$(document).on("click", ".openImageDialog", function () {
    ...
    let myImageName = $(this).data('judul');
...
// function load_table() {
	...
	var table =	$("#table1").DataTable({
					...
					ajax : {
						...
						url: base_url + 'admin/Manage_content/dt_image_loan',
...
function save_data() {
	...
	$.ajax({
		...
		url : base_url+"admin/Content_trx/ajax_simpan_image_loan",
...
function get_id(id) {
	...
	$.ajax({
		...
		url: base_url + "admin/Content_trx/get_imageloan_by_id/" + id,
	...
	bootbox.confirm("Yakin akan menghapus data ?", function(event){
		...
		if (event == true) {
			...
			$.ajax({
				...
				url : base_url+'admin/Content_trx/hapus_imageloan/'+id,
