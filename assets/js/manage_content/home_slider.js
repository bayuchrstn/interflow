...
var table =	$("#table1").DataTable({
				...
				responsive: true,
				ajax : {
					url: base_url + 'admin/Manage_content/dt_home_slider',
					type: "GET"
				},
				columnDefs: [
					{
...
var myDropzone = new Dropzone("#slider-upload",{
					paramName: "file_slider", // The name that will be used to transfer the file
					maxFilesize: 0.25, // MB
					maxFiles: 10,
					acceptedFiles: "image/*",
...
function save_data() {
	...
	$.ajax({
		url : base_url+"admin/Content_trx/ajax_simpan_home_slider",
		type : "POST",
		data : formData,
		dataType : "JSON",
		contentType: false,
...
function get_id(id) {
	...
	$.ajax({
		url: base_url + "admin/Content_trx/get_home_slider_by_id/" + id,
		dataType: "json",
		beforeSend: function () {
			blockUI();
		},
...
function delete_data(id) {
	bootbox.confirm("Yakin akan menghapus data ?", function(event){
		if (event == true) {
			$.ajax({
				url : base_url+'admin/Content_trx/hapus_home_slider/'+id,
				dataType : "json",
				beforeSend : function(){
					blockUI();
				},
