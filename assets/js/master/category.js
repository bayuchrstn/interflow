...
// function load_table() {
    var table =	 $("#table1").DataTable({
                    ...
                    responsive: true,
                    ajax : {
                        url: base_url + 'admin/Master/dt_category',
                        type: "GET"
                    },
                    columnDefs: [
                        {
...
$('#form-data').submit(function (event) { 
    ...
    $.ajax({
		url: base_url+"admin/Master/ajax_simpan_category",
		type: "post",
		data: formData,
		async: false,
		cache: false,
...
function get_id(id) {
	$.ajax({
		url: base_url + "admin/Master/category_id/" + id,
		dataType: "json",
		beforeSend: function () {
			blockUI();
		},
...
function delete_data(id) {
    bootbox.confirm("Yakin akan menghapus data ?", function(event){
        if (event == true) {
            $.ajax({
                url : base_url+'admin/Master/hapus_category/'+id,
                dataType : "json",
                beforeSend : function(){
                    blockUI();
                },
