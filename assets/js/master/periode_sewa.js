...
var table =   $("#table1").DataTable({
                ...
                responsive: true,
                ajax : {
                    url: base_url + 'admin/Master/dt_periode_sewa',
                    type: "GET"
                },
                columnDefs: [
                    {
...
$('#form-data').submit(function (event) { 
    ...
    $.ajax({
		url: base_url+"admin/Master/ajax_simpan_periode_sewa",
		type: "post",
		data: formData,
		async: false,
		cache: false,
...
function get_id(id) {
	$.ajax({
		url: base_url + "admin/Master/periode_sewa_id/" + id,
		dataType: "json",
		beforeSend: function () {
			blockUI();
		},
...
function delete_data(id) {
    bootbox.confirm("Yakin akan menghapus data ?", function(event){
        if (event == true) {
            $.ajax({
                url : base_url+'admin/Master/hapus_periode_sewa/'+id,
                dataType : "json",
                beforeSend : function(){
                    blockUI();
                },
