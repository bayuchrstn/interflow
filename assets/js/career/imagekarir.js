...
// function load_table() {
	...
	var table =	$("#table1").DataTable({
					...
					ajax : {
						...
						url: base_url + 'admin/Career/dt_image',
...
function save_data() {
	...
	$.ajax({
		...
		url : base_url+"admin/Career/ajax_simpan_image",
...
function get_id(id) {
	...
	$.ajax({
		...
		url: base_url + "admin/Career/get_image_by_id/" + id,
	...
	bootbox.confirm("Yakin akan menghapus data ?", function(event){
		...
		if (event == true) {
			...
			$.ajax({
				...
				url : base_url+'admin/Career/hapus_image/'+id,
