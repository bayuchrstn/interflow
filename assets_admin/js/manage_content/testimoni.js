...
function load_table() {
	...
	$("#table1").DataTable({
        ...
        responsive: true,
        ajax : {
            url: base_url + 'admin/Manage_content/dt_testimoni',
            type: "GET"
        },
        columnDefs: [
            {
...
function save_data() {
	...
	$.ajax({
		url : base_url+"admin/Content_trx/ajax_simpan_testimoni",
		type : "POST",
		data : formData,
		dataType : "JSON",
		contentType: false,
...
function get_id(id) {
	...
	$.ajax({
		url: base_url + "admin/Content_trx/get_testimoni_by_id/" + id,
		dataType: "json",
		beforeSend: function () {
			blockUI();
		},
...
function delete_data(id) {
	bootbox.confirm("Yakin akan menghapus data ?", function(event){
		if (event == true) {
			$.ajax({
				url : base_url+'admin/Content_trx/hapus_testimoni/'+id,
				dataType : "json",
				beforeSend : function(){
					blockUI();
				},
