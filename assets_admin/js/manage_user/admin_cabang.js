...
function input_data() {
	$("#modal_admin_cabang").attr('transaksi', 'tambah');
	$('#modal_admin_cabang').modal('show');
}
...
function load_table() {
	...
	$("#table1").DataTable({
		...
		"serverSide": true,
		"ajax": {
			"url": base_url + 'admin/Manage_user/get_data_admin_cabang',
			"type": "POST",
			"data": function(data) {
...
function save_data() {
	...
	$.ajax({
		url : base_url+"admin/Manage_user/ajax_simpan_admin_cabang",
		type : "POST",
		data : formData,
		dataType : "JSON",
		contentType: false,
